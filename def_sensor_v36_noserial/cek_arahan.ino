...
void cek_arah_tangga_normal(int max_looping) { // untuk rintangan kelereng
  int arah_benar = 0;
  int left_bearing = 355;
...
void cek_arah_empty_serong_knn(int max_looping) { // untuk rintangan empty, sengaja serong kanan dikit supaya gak kena dinding K2
  int arah_benar = 0;
  int left_bearing = 285;
  int right_bearing = 295;
  int looping = 0;
...
//  }
  ...
  while (!geser_tengah) {
    ...
    if (jarak_knn >= 19 && jarak_knn <= 21) {
      home1();
      geser_tengah = 1;
    } else if (jarak_knn < 19) {
      geser_kiri();
    } else if (jarak_knn > 21) {
      geser_kanan();
    }
  }
